jQuery(document).ready(function(){
    ...
    $.post(Form.action(), Form.data(), function(data){
        ...
        if($return === true){
			...
			$('#contactForm input[type="text"]').attr('value', '');
            $('#contactForm textarea[name="message"]').attr('value', '');
            $('#contactForm img').attr('src', 'assets/img/contact/ajax-loader.gif');
            $('#contactForm img').attr('src', 'assets/php/security/1/sec.php?'+Math.random());
